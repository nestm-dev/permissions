...
import {
	...
	createProviderDefinitionReadinessProvider,
...
export class PermissionsModule extends ConfigurableModuleClass implements OnModuleInit {
	...
	static override forRoot(options: PermissionsForRootOptions): DynamicModule {
		...
		const storeReadiness = createProviderDefinitionReadinessProvider(
			POLICY_STORE_DEFINITION_DEPENDENCIES_READY,
			options.store,
		);
		const principalResolverReadiness = createProviderDefinitionReadinessProvider(
			PRINCIPAL_RESOLVER_DEFINITION_DEPENDENCIES_READY,
			options.principalResolver,
		);
		return {
			...definition,
			providers: [...(definition.providers ?? []), storeReadiness, principalResolverReadiness],
		};
	}
