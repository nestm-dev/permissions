...
//
// Note on duplication: `@nestm/permissions-drizzle` has a component with the same
// contract. It is not shared, because the only two places to share it from are
// core — whose `PolicyStore` SPI deliberately says nothing about *how* a driver
// notices a change, and which would have to grow a timer abstraction to host this
// — and a new package, which the design rules out. The behaviour is pinned by
// tests on both sides rather than by a common implementation.
 ...
 *
 * Separate from TypeORM's pool by construction (the caller supplies the client),
 * and separate from the poller by design: `NOTIFY` is best-effort — it is lost if
 * no one is listening at that instant, and a dropped connection loses every
 * notification until it reconnects — so it *accelerates* invalidation rather than
 * replacing the poll. Leaving `poll: false` alongside `notify` is supported but
 * means a missed notification is never recovered.
 */
