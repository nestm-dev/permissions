...
//
// The shapes are the *property* names of the `EntitySchema`s, not the SQL column
// names. That is the stable surface: a consumer writes `row.cedarJson` whatever
// their scope column or table prefix is called.
...
/** One `${prefix}policies` row, as TypeORM hands it back. */
export interface PolicyRow {
	scope: unknown;
	policyId: string;
	kind: string;
	cedarJson: PolicyJson;
	cedarText: string | null;
...
/** One `${prefix}policy_links` row, as TypeORM hands it back. */
...
/**
 * One `${prefix}scope_versions` row.
 *
 * `version` is declared `bigint`, and TypeORM's Postgres driver hands `int8` back
 * as a **string** — so it is typed as the union rather than as a `number`, and
 * every read goes through `Number(...)`. A driver that assumed `number` would
 * compose `g[object Object]:s…` versions and invalidate nothing.
 */
...
/** Validates a record the way `MemoryPolicyStore` does, so every store rejects the same inputs. */
...
/** Projects a {@link PolicyRecord} onto the properties of the policies entity. */
export function policyRowValues(record: PolicyRecord): PolicyRowValues {
	return {
		...
		// returns, and TypeORM serialises lazily.
...
/** Projects a {@link TemplateLinkRecord} onto the properties of the links entity. */
