...
//   (c) `selected`  — `SELECT id FROM docs WHERE <applyPlan(...)>` against **real
//       Postgres**, through a `SelectQueryBuilder`. This is what an application
//       actually gets.
//
...
//
// The suite runs against **three** mappings over one ground truth, so the four
// hierarchy strategies and the two attribute-access paths have to agree with each
// other as well as with Cedar:
//
//   A  column + recursive (row), self + closure (attribute)
//   B  closure everywhere (row), self + recursive (attribute)
//   C  every scalar through `meta` jsonb — `#>>` plus a cast, per value kind
//
// A compiler that got reflexivity right in one strategy and wrong in another, or
// that read a `bigint` correctly from a column and as text from JSON, shows up
// here rather than in production.
//
// Run count is env-tunable:
//   TYPEORM_DIFFERENTIAL_RUNS=100 pnpm --filter @nestm/permissions-typeorm test tests/property
...
const RUNS = Number.parseInt(process.env["TYPEORM_DIFFERENTIAL_RUNS"] ?? "12", 10);
...
const SCHEMA = uniqueSuffix("nestm_diff");
const PREFIX = "d_";

let cedar: CedarBinding;
let dataSource: DataSource;
let entities: DocEntities;
let mappings: readonly { name: string; mapping: TypeOrmResourceMapping<"Doc"> }[];
...
const leafCondition: fc.Arbitrary<string> = fc.oneof(
	...
	fc
		...
		.map((set) => `[${set.map((status) => `"${status}"`).join(",")}].contains(resource.status)`),
	fc
		.tuple(fc.constantFrom("<", "<=", ">", ">="), fc.integer({ min: 0, max: 20 }))
		.map(([operator, size]) => `resource.size ${operator} ${String(size)}`),
	...
	fc
		...
		.map(
			([operator, at]) =>
				`resource has publishedAt && resource.publishedAt ${operator} datetime("${at}")`,
		),
	fc
		.tuple(fc.constantFrom("<", "<=", ">", ">=", "==", "!="), fc.integer({ min: 0, max: 20 }))
		.map(
			([operator, score]) => `resource has score && resource.score ${operator} ${String(score)}`,
		),
	...
	fc
		...
		.map(([operator, rate]) => `resource has rate && resource.rate ${operator} decimal("${rate}")`),
	// INET: address + prefix, Cedar's rule and Postgres's.
	fc
		.tuple(fc.constantFrom("==", "!="), fc.constantFrom(...ADDRESSES))
		.map(([operator, addr]) => `resource has addr && resource.addr ${operator} ip("${addr}")`),
	fc
		.tuple(fc.constantFrom("<", "<=", ">", ">=", "==", "!="), fc.constantFrom(...TTLS))
		.map(
			([operator, ms]) =>
				`resource has ttl && resource.ttl ${operator} duration("${String(ms)}ms")`,
		),
	...
	fc.integer({ min: 0, max: 40 }).map((index) => `resource in Test::Doc::"d${String(index)}"`),
);
...
const conditionArb: fc.Arbitrary<string> = fc.letrec<{ condition: string }>((tie) => ({
	condition: fc.oneof(
		...
		fc
			...
			.map(([test, then, otherwise]) => `if (${test}) then (${then}) else (${otherwise})`),
	),
})).condition;
...
async function makeEngine(
	...
	const engine = await createEngine<DocVocabulary>({
		...
		instanceId: `typeorm-diff-${String(process.pid)}-${String(instanceCounter)}`,
		cedar,
	});
	engines.push(engine);
	return engine;
...
/** The third row set: real Postgres, through the compiled `WHERE` clause. */
async function selectedIds(
	plan: Parameters<typeof applyPlan>[1],
	mapping: TypeOrmResourceMapping<"Doc">,
): Promise<Set<string>> {
	const query = dataSource.createQueryBuilder(entities.docs, "doc").select("doc.id", "id");
	const rows = await applyPlan(query, plan, mapping).orderBy("doc.id").getRawMany<{ id: string }>();
...
describe.skipIf(PG_SKIPPED)("three-way differential", () => {
	beforeAll(async () => {
		...
		mappings = [
			{ name: "column+recursive", mapping: docMappingColumnAndRecursive(dataSource, entities) },
			{ name: "closure+recursive", mapping: docMappingClosureAndRecursive(dataSource, entities) },
			{ name: "jsonPath", mapping: docMappingJsonPath(dataSource, entities) },
		];
	});
	...
	it(`selects exactly the rows check() allows (${String(RUNS)} runs, 3 mappings each)`, async () => {
		await fc.assert(
			fc.asyncProperty(scenarioArb, async (scenario) => {
				const { records, links, sources } = buildPolicies(scenario.policies);
				const engine = await makeEngine(records, links);

				await seedDocs(dataSource, entities, scenario.rows);
				await seedHierarchy(dataSource, entities, scenario.rows);
				...
				for (const { name, mapping } of mappings) {
					const selected = await selectedIds(plan, mapping);
